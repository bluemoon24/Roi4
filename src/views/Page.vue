...
<script>
  ...
  export default {
    ...
    created: function () {
      ...
      this.cwidth = Math.max(960, window.innerWidth) * 0.98 - this.rwidth
      this.cheight =  Math.max(500, window.innerHeight) * 0.98 - this.nheight
    },
    ...
    methods: {
      ...
      drawPortlet: function (tile, mode) {
        ...
        var content = tile.data()[0].content;
        let layoutrect = tile.data()[0].layout.content
        console.log('******************* drawPortlet **************', content.id, layoutrect, tile.data()[0])
        if (!content)
        ...
        //   return color (d.content.id) })
          ...
          layoutrect = contdiv.getBoundingClientRect()
